from flask import Flask
from flask_cors import CORS, cross_origin
import json
# send email to subscribed user
import smtplib
from email.mime.multipart import MIMEMultipart
from email.mime.text import MIMEText

import os
import requests

app = Flask(__name__, static_folder='frontend/build', static_url_path='')
CORS(app)

def subscribe(emailid):
    SUPABASE_URL = os.getenv('SUPABASE_URL')
    SUPABASE_HEADERS = {
        'apikey': os.getenv('SUPABASE_API_KEY'),
        'Authorization': 'Bearer' + os.getenv('SUPABASE_API_KEY'),
        'Content-Type': 'application/json',
        'Prefer': 'return=representation'
    }
    try:
        data = {'emailid': emailid}
        result = requests.post(f'{SUPABASE_URL}/newsletter', headers=SUPABASE_HEADERS, json=data)
        if(result):
            # Send email to subscribed User
            subject = "Welcome to Cryptocurrency MarketPlace Newsletter"
            body = f"""
                <h3>Thank you! You have subscribed for Cryptocurrency MarketPlace Newsletter.</h3>
                <p>You will receive occasional Crypto currency news, updates, and reports that will help you to learn about Crypto currency.</p>
                <p>Cheers,<br/>Cryptocurrency Admin Team,<br/>Dublin 1.</p>
            """
            send_email('shricryptoblog@gmail.com',
                    'Shree@2@21', emailid, subject, body)
            response = {'success': True, 'data': result.text}
            retData = app.response_class(
                response=json.dumps(response),
                status=200,
                mimetype='application/json'
            )
            return retData;
        else:
            return {'success': False, 'status': 500, 'message': 'Unauthorized User. Please try again!'}, 500
    except Exception as e:
        return {'success': False, 'message': str(e)}, 500

def send_email(gmailUser, gmailPassword, recipient, subject, body):
    msg = MIMEMultipart()
    msg['From'] = f'"Cryptocurrency MarketPlace Admin " <shricryptoblog@gmail.com>'
    msg['To'] = recipient
    msg['Subject'] = subject
    msg.attach(MIMEText(body, 'html'))

    try:
        mailServer = smtplib.SMTP('smtp.gmail.com', 587)
        mailServer.ehlo()
        mailServer.starttls()
        mailServer.ehlo()
        mailServer.login(gmailUser, gmailPassword)
        mailServer.sendmail(gmailUser, recipient, msg.as_string())
        mailServer.close()
    except LookupError:
        print('Something went wrong...', LookupError)
